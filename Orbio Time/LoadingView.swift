...
extension LoadingView {
    ...
    func req() {
        ...
        conversionData["bundle_id"] = "com.app.orbitotimeorb"
        conversionData["os"] = "iOS"
        conversionData["store_id"] = "6756574526"
        conversionData["locale"] = Locale.current.identifier
        conversionData["firebase_project_id"] = "718521649088"

        do {
            let jsonData = try JSONSerialization.data(withJSONObject: conversionData, options: [])
                    let url = URL(string: "https://orbiotime.com/config.php")!
